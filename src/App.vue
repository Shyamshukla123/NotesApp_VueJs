<script setup>
  ...
  const notes = ref(JSON.parse(localStorage.getItem('myNotes') || '[]'));
...
const storeNotes = (newNotes) => {
  localStorage.setItem('myNotes', JSON.stringify(newNotes))
}

// Example function to add a new note to the array
const addNotes = () => {
  if(newNote.value.length<10){
      return errorMessage.value="note should be 10 character or more";
    }
    ...
    notes.value.push({
      ...
      newNote.value="";
      errorMessage.value="";
      storeNotes(notes.value)
}

onMounted(() => {
  console.log(notes.value) // Display the stored notes array in the console
})
